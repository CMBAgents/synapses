...
function ChatTemplateContent({ domain }: ChatTemplateProps) {
  ...
  const getDefaultProgramIdForDomain = () => {
    const defaultPrograms: Record<string, string> = {
      'astronomy': 'skyfielders-python-skyfield',
      'finance': 'quantopian-zipline',
      'biochemistry': 'biopython-biopython',
      'machinelearning': 'pytorch-pytorch',
      'computer-science': 'numpy-numpy'
    };
    return defaultPrograms[domain] || 'skyfielders-python-skyfield';
  };
  ...
  useEffect(() => {
    ...
    if (urlProgramId) {
      ...
      try {
        ...
        // If domain doesn't exist, use default
        programId = getDefaultProgramIdForDomain();
      }
    }
  ...
  try {
    ...
    // Fallback to empty data if domain doesn't exist
    domainData = { libraries: [], description: '' };
  }
...
export default function ChatTemplate({ domain }: ChatTemplateProps) {
  return (
    <Suspense fallback={<div>Loading...</div>}>
      <ChatTemplateContent domain={domain} />
    </Suspense>
  );
}
