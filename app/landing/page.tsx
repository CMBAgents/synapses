    ...
    >
      ...
      <div className="relative z-10 min-h-screen flex flex-col items-center justify-center px-4 text-white page-fade-in">
        ...
        <div className="max-w-6xl w-full content-fade-in px-2">
          {/* Machine Learning au centre en haut */}
          <div className="flex justify-center mb-4 sm:mb-6">
            {domains.filter(domain => domain.id === 'machine-learning').map((domain) => {
              const isDisabled = true; // ML est désactivé pour l'instant
              
              return (
                <div
                  key={domain.id}
                  className={`relative group bg-white/10 backdrop-blur-sm border border-white/20 rounded-xl transition-all duration-300 transform hover:scale-105 hover:bg-white/20 hover:border-white/30 shadow-lg hover:shadow-xl w-full max-w-xs sm:w-64 ${
          ...
          {/* Astronomy et Finance en bas */}
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 max-w-3xl mx-auto">
            {domains.filter(domain => domain.id !== 'machine-learning').map((domain, index) => {
